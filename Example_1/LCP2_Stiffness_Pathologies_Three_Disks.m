...
%
%  Column 5:  du as a N x 2 array
%  Column 6:  dtheta as a N x 1 array
%
%
...
%    Column 13:  db (an Nx2 arrangement of dy)
%    Column 14:  dw (an Nx1 arrangement of dy)
%
  Details = cell(500,14);
  nDetails = 0;
%
...
%             a consistent single solution
              Affine = Affine_Matrix \ Null_Space(1:2*N,:);
              if all(S_pq_forward(Subset)==s_pq(Subset))
                Strain =  Affine(3:6);
              else
                Strain = -Affine(3:6);
...
%  
                ...
                Affine = Affine_Matrix \ Null_Space(1:2*N,i);
                if all(S_pq_forward(Subset,i)==s_pq(Subset))
                  Strain = [Strain,  Affine(3:6)];
                else
                  Strain = [Strain, -Affine(3:6)];
                end
              end
...
%               normalize the combination vector X to unit length
                X = (1 / sqrt(sum(X.^2))) * X;
%  
                Affine = Affine_Matrix \ (Null_Space(1:2*N,:)*X);
                Strain = Affine(3:6);
%  
...
%  
%             a consistent single solution
              Affine = Affine_Matrix \ Neg_Eigenvectors(1:2*N,:);
              if all(S_pq_forward(Subset)==s_pq(Subset))
                Strain =  Affine(3:6);
              else
                Strain = -Affine(3:6);
...
%  
                Found_Consistent_Basis_Vectors = ...
                  Found_Consistent_Basis_Vectors + 1;
                Affine = Affine_Matrix \ Neg_Eigenvectors(1:2*N,i);
                if all(S_pq_forward(Subset,i)==s_pq(Subset))
                  Strain = [Strain,  Affine(3:6)];
                else
                  Strain = [Strain, -Affine(3:6)];
...
%               normalize the combination vector X to unit length
                X = (1 / sqrt(sum(X.^2))) * X;
%  
                Affine = Affine_Matrix \ (SubSpace(1:2*N,:)*X);
                Strain = Affine(3:6);
%  
