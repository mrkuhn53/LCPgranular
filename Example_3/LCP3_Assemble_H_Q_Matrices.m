...
function  [H_x, H_m_x, H_g1, H_g2, H_g3, H_g1234, B, ...
          ...
          LCP3_Assemble_H_Q_Matrices( ...
            ...
            N, FM_model_pq, lambda_tolerance, m_lambdas);
%
...
% variant:
  [S1,I1] = sortrows([V_pq, C_pq], [1,2,3,4]);
  [S2,I2] = sortrows([V_pq(:,[2 1]),C_pq(:,[2 1])], [1,2,3,4]);
%
...
% initialize the stiffness matrices
  H_m_x =                   zeros(6*N, 6*N+6);      % append 6 dxcell's
  H_lambda =                zeros(6*N,M_lambda);
  Q_x =                     zeros(M_lambda, 6*N+6); % append 6 dxcell's
  Q_lambda =                zeros(M_lambda, M_lambda);
  H_g1 =                    zeros(6*N, 6*N+6);      % append 6 dxcell's
  H_g2 =                    zeros(6*N, 6*N+6);      % append 6 dxcell's
  H_g3 =                    zeros(6*N, 6*N+6);      % append 6 dxcell's
  H_g1234 =                 zeros(6*N, 6*N+6);      % append 6 dxcell's
  map_lambda_to_pq_and_qp = zeros(M_lambda,2);
  map_pq_to_lambdas =       zeros(2*M,m_lambdas);
% B =                       zeros(2*M*6,6*N);   % ZZZ
  B =                       zeros(2*M*6,6*N+6); % add 6 columns for dxcell
%
...
%         lambdas within the assembly
          M_lambda = M_lambda + 1;
%
%         both pq and qp share the same lambda
          map_pq_to_lambdas(pq,k) = M_lambda;
          map_pq_to_lambdas(map_pq_to_qp(pq),k) = M_lambda;
%
%         map the current M_lambda to the pq and qp contact variants
          map_lambda_to_pq_and_qp(M_lambda,:) = [pq, map_pq_to_qp(pq)];
        else
%         the lambda has already been created for qp, so assign it to pq
          M_lambda = map_pq_to_lambdas(pq,k);
        end
%
