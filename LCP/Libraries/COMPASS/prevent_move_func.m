function [ t, j, Var, no_move_count, ...
...
%
% An adaptation of the prevent_move.m file of Stefan Schmelzer, in the
    ...
    no_move_count = no_move_count+1; 
      ...
      while j_already_used == 1
        ...
        if ((j_already_used == 1) && (k < length(j_alternative)))
          k = k+1;
          j = j_alternative(k); 
          % take the smallest possible new j to leave. If they
    ...
    if no_move_a_count > 1
      ...
      while a_already_used == 1
        ...
        if ((a_already_used == 1) && (k < length(a_alternative)))
          k = k+1;
          j = a_alternative(k); 
              % take the smallest possible new a to leave. If they
              % were all already used, take the first and, clear
              % the memory.
