...
%
          if all(NullAr  > -EPS2) || all(NullAr  < EPS2)
...
%
          if any(all(NullAr  > -EPS2) | all(NullAr  < EPS2))
...
%
            iNullAr = find(all(NullAr  > -EPS2) | all(NullAr  < EPS2));
%
            for i = iNullAr
              if all(NullAr(:,i)  < EPS2)
                NullAr(:,i) = -NullAr(:,i);
              end
%
%             the zero and non-negative "x" values
