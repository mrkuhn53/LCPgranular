...
%       currently, we only provide for a rank of Ar == n-1
      elseif rank(Ar,EPS1)==(n-1)
%       the orthogonal complement of Inv_Ar
        Perp_Ar = eye(n) - Inv_Ar*Ar;
%
%       rank 1 basis vector of Perp_Ar
        t = orth(Perp_Ar, EPS2);
%
        Sign = 0;
        iNeg = find(y<-EPS2);
        if all(sign(t(iNeg)) == -1)
          Sign = 1;
          Factor = max(y(iNeg)./t(iNeg));
        elseif all(sign(t(iNeg)) == 1)
          Sign =  1;
          Factor = min(y(iNeg)./t(iNeg));
        end
%
        if Sign && all(y - Factor*t  >= -EPS2)
          x = zeros(n,1);
          x(Indices) = y(Indices) - Factor*t(Indices);
          X = [X x];
          DegenSoln = [DegenSoln any(y==0)];
        end
%
