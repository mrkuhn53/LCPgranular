...
function [H_m_pq_x, H_g1_pq, H_g2_pq, H_g3_pq, B_pq, H_m_max, ...
  ...
  LCP_Stiffness_Matrices_pq(r_pq_p, r_pq_q, n_pq, f_pq, m_pq, ...
...
% stiffnesses.
  H_m_pq_x =         zeros(2*M,12,12);
  H_pq_lambda =    zeros(2*M,12,m_lambdas);
  Q_pq_x_all =       zeros(2*M,m_lambdas,12);
  Q_pq_lambda_all =  zeros(2*M,m_lambdas,m_lambdas);
  k_lambdas_pq_all = zeros(2*M);
  lambdas_pq_all =   zeros(2*M,m_lambdas);
  H_g1_pq =          zeros(2*M,12,12);
  H_g2_pq =          zeros(2*M,12,12);
  H_g3_pq =          zeros(2*M,12,12);
  B_pq =             zeros(2*M,6,12);
%
...
%   on the activation of slip parameters "lambda"
    [F_pq, M_pq, S_pq_x, S_pq_lambda, Q_pq_x, Q_pq_lambda, lambdas_pq, ...
     k_lambdas_pq] = ...
     LCP_F_M_Q_pq(FM_model_pq(pq), n_pq(pq,:), f_pq(pq,:), m_pq(pq,:), ...
...
%   6x12 matrix E_g1_pq.  The first 3 rows give a force effect; the lower
%   3 rows give the moment effect.  The columns are arranged as
%   follows: rows 1:3 are displacements of p; rows 4:6 are displacements
%   of q; rows 7:9 are rotations of p; rows 10:12 are rotations of q.
%   Note that the first 3 rows of this matrix are filled with zeros, 
%   since the g-1 stiffness applies only to moments and not to forces 
%   (note that the g-1 term is absent in KDP Eq. 15)
    E_g1_pq = [ zeros(3,12); ...
...
%
%   the first 3 rows give a force effect; the lower 3 rows give the moment
%   effect.  The columns are arranged as follows: rows 1:3 are displacements 
%   of p; rows 4:6 are displacements of q; rows 7:9 are rotations of p; 
%   rows 10:12 are rotations of q.
    E_g3_pq = [ zeros(3), zeros(3), -Cross_Product(f_pq(pq,:)), zeros(3); ...
                zeros(3), zeros(3), -Cross_Product(m_pq(pq,:)), zeros(3) ];
%   note that the function "Cross_Product" effects the product a X b,
%   so a change in sign is required for dtheta X f_pq
%
...
%
    Eye = [ -eye(3), zeros(3); ... % p force, neg sign is from KPD Eqs 15, 28
             eye(3), zeros(3); ... % q force, ignored during assembly
           zeros(3),  -eye(3); ... % p moment, neg sign is from KPD Eqs 16, 28
           zeros(3),   eye(3)];    % q moment, ignored during assembly
%
